...
class EnhancedPatternRenderer(BaseRenderer):
    ...
    def _points(cx, cy, size, settings, rng, rotation):
        style = settings["style"]; lobes = max(3, int(settings["lobes"]))
        wobble = max(0.0, min(1.0, float(settings["wobble"]))); taper = max(0.0, min(1.0, float(settings["taper"])))
        count = max(20, lobes * 5); phase = rng.uniform(0, math.tau)
        ...
        for i in range(count):
            ...
            elif style == "shell": r *= 0.72 + 0.22*(i/max(1,count-1)) + 0.12*math.sin(3*a + phase)
            elif style == "seed": r *= 0.78 + 0.32*(0.5 + 0.5*math.cos(a))
    ...
    def _draw_text_extension(self, result, config):
        ...
        if mode == "grid":
            ...
            for i, char in enumerate(chars):
                a = start + i * step
                paint_char(char, cx + math.cos(a)*radius, cy + math.sin(a)*radius, math.degrees(a) + 90 + settings["rotation"])
        elif mode == "spiral":
            ...
            for i, char in enumerate(chars):
                ...
                paint_char(char, cx + math.cos(a)*r, cy + math.sin(a)*r, math.degrees(a) + 90 + settings["rotation"])
        elif mode == "scatter":
            for char in chars:
                paint_char(char, palette_rng.uniform(0.06, 0.94)*config.width, palette_rng.uniform(0.08, 0.92)*config.height, palette_rng.uniform(-24, 24) + settings["rotation"])
        elif mode == "wave":
            x = settings["font_size"]; baseline = config.height * 0.5
            for i, char in enumerate(chars):
                if x >= config.width - settings["font_size"]:
                    x = settings["font_size"]; baseline += line_height * 1.4
...
class MainWindow(BaseMainWindow):
    ...
    def _build_ui(self):
        ...
        if export_index >= 0 and export_index != tabs.count() - 1:
            widget = tabs.widget(export_index); title = tabs.tabText(export_index)
            tabs.removeTab(export_index); tabs.addTab(widget, title)
    ...
    def _organic_tab(self):
        ...
        for n,w in [("Enable",self.use_organic),("Style",self.organic_style),("Weight",self.organic_weight),("Lobes",self.organic_lobes),("Wobble",self.organic_wobble),("Taper",self.organic_taper),("Linework",self.organic_line_mode),("Vein count",self.organic_vein_count),("Vein wobble",self.organic_vein_wobble),("Long strands",self.organic_strand_count),("Strand length",self.organic_strand_length),("Strand wander",self.organic_strand_wander),("Strand width",self.organic_strand_width)]: form.addRow(n,w)
        layout.addWidget(box)
        hint = QLabel("Organic linework adds smooth stochastic veins, tendrils and field-guided strands while remaining deterministic from the seed.")
        hint.setWordWrap(True); layout.addWidget(hint); layout.addStretch(1); return page
    ...
    def _add_perspective_controls(self, tabs):
        ...
        for n,w in [("Enable",self.perspective_enabled),("Depth",self.perspective_strength),("Vanishing X",self.vanishing_x),("Vanishing Y",self.vanishing_y),("Edge light",self.edge_light)]: form.addRow(n,w)
    ...
    def _text_tab(self):
        ...
        self.text_input = QPlainTextEdit(); self.text_input.setPlaceholderText("Sentence, multiline text, hiragana, katakana, kanji, symbols…"); self.text_input.setMinimumHeight(130)
        self.text_mode = QComboBox(); self.text_mode.addItems(["flow","wave","arc","spiral","grid","scatter"])
        self.text_font = QComboBox(); self.text_font.addItems([_system_font_family(), "Sans Serif"]); self.text_font.setCurrentText(_system_font_family())
        ...
        for n,w in [("Enable",self.use_text),("Text",self.text_input),("Arrangement",self.text_mode),("Font",self.text_font),("Size",self.text_font_size),("Opacity",self.text_opacity),("Tracking",self.text_tracking),("Line spacing",self.text_line_spacing),("Radius",self.text_radius),("Wave amount",self.text_wave),("Rotation",self.text_rotation)]: form.addRow(n,w)
        layout.addWidget(box)
        hint = QLabel("Text is rendered by Qt, so Japanese and mixed Unicode can use OS fallback fonts. Bahnschrift is preferred for Latin text when installed.")
        hint.setWordWrap(True); layout.addWidget(hint); layout.addStretch(1); return page
    ...
    def _wire_extensions(self):
        controls = [
            self.use_organic,self.organic_style,self.organic_weight,self.organic_lobes,self.organic_wobble,self.organic_taper,
            self.organic_line_mode,self.organic_vein_count,self.organic_vein_wobble,self.organic_strand_count,self.organic_strand_length,
            self.organic_strand_wander,self.organic_strand_width,self.preserve_palette,self.perspective_enabled,self.perspective_strength,
            self.vanishing_x,self.vanishing_y,self.edge_light,self.use_text,self.text_input,self.text_mode,self.text_font,self.text_font_size,
            self.text_opacity,self.text_tracking,self.text_line_spacing,self.text_radius,self.text_wave,self.text_rotation,
        ]
    ...
    def _config(self):
        ...
        self.renderer.organic_settings = {
            ...
            "line_mode":self.organic_line_mode.currentText(),"vein_count":self.organic_vein_count.value(),"vein_wobble":self.organic_vein_wobble.value(),
            "strand_count":self.organic_strand_count.value(),"strand_length":self.organic_strand_length.value(),"strand_wander":self.organic_strand_wander.value(),"strand_width":self.organic_strand_width.value(),
        }
        self.renderer.perspective_settings = {"enabled":self.perspective_enabled.isChecked(),"strength":self.perspective_strength.value(),"vanishing_x":self.vanishing_x.value(),"vanishing_y":self.vanishing_y.value(),"edge_light":self.edge_light.value()}
        self.renderer.text_settings = {"enabled":self.use_text.isChecked(),"text":self.text_input.toPlainText(),"mode":self.text_mode.currentText(),"font_family":self.text_font.currentText(),"font_size":self.text_font_size.value(),"opacity":self.text_opacity.value(),"tracking":self.text_tracking.value(),"line_spacing":self.text_line_spacing.value(),"radius":self.text_radius.value(),"wave":self.text_wave.value(),"rotation":self.text_rotation.value()}
    ...
    def save_preset(self):
        ...
        data["_eli_lab_extensions"] = {"organic":self.renderer.organic_settings,"perspective":self.renderer.perspective_settings,"text":self.renderer.text_settings,"preserve_palette":self.preserve_palette.isChecked()}
    ...
    def load_preset(self):
        ...
        if not path: return
        try:
            data = json.loads(Path(path).read_text(encoding="utf-8")); ext = data.pop("_eli_lab_extensions", {})
            cfg = PatternConfig(**data).normalized(); self._loading_preset = True
            self.width.setValue(cfg.width); self.height.setValue(cfg.height); self.seed.setText(cfg.seed); self.background.setText(cfg.background); self.aspect.setCurrentText("custom"); self.symmetry.setCurrentText(cfg.symmetry)
            self.palette_mode.setCurrentIndex(max(0,self.palette_mode.findData(cfg.palette_mode))); self.composition_mode.setCurrentText(cfg.composition_mode); self.focal_x.setValue(cfg.focal_x); self.focal_y.setValue(cfg.focal_y); self.focal_strength.setValue(cfg.focal_strength); self.edge_bias.setValue(cfg.edge_bias); self.cluster_count.setValue(cfg.cluster_count); self.cluster_strength.setValue(cfg.cluster_strength); self.spacing.setValue(cfg.spacing); self.jitter.setValue(cfg.jitter)
            ...
            o = ext.get("organic", {})
            if o:
                self.use_organic.setChecked(bool(o.get("enabled",False))); self.organic_style.setCurrentText(o.get("style","amoeba")); self.organic_weight.setValue(float(o.get("weight",0.8))); self.organic_lobes.setValue(int(o.get("lobes",7))); self.organic_wobble.setValue(float(o.get("wobble",0.55))); self.organic_taper.setValue(float(o.get("taper",0.25))); self.organic_line_mode.setCurrentText(o.get("line_mode","mixed")); self.organic_vein_count.setValue(int(o.get("vein_count",3))); self.organic_vein_wobble.setValue(float(o.get("vein_wobble",0.55))); self.organic_strand_count.setValue(int(o.get("strand_count",8))); self.organic_strand_length.setValue(int(o.get("strand_length",26))); self.organic_strand_wander.setValue(float(o.get("strand_wander",0.45))); self.organic_strand_width.setValue(float(o.get("strand_width",1.5)))
            p = ext.get("perspective", {})
            if p:
                self.perspective_enabled.setChecked(bool(p.get("enabled",True))); self.perspective_strength.setValue(float(p.get("strength",0.18))); self.vanishing_x.setValue(float(p.get("vanishing_x",0.5))); self.vanishing_y.setValue(float(p.get("vanishing_y",0.42))); self.edge_light.setValue(float(p.get("edge_light",0.28)))
            t = ext.get("text", {})
            if t:
                self.use_text.setChecked(bool(t.get("enabled",False))); self.text_input.setPlainText(t.get("text","")); self.text_mode.setCurrentText(t.get("mode","flow")); self.text_font.setCurrentText(t.get("font_family",_system_font_family())); self.text_font_size.setValue(int(t.get("font_size",72))); self.text_opacity.setValue(float(t.get("opacity",0.82))); self.text_tracking.setValue(float(t.get("tracking",2))); self.text_line_spacing.setValue(float(t.get("line_spacing",1.18))); self.text_radius.setValue(float(t.get("radius",0.33))); self.text_wave.setValue(float(t.get("wave",0.12))); self.text_rotation.setValue(float(t.get("rotation",0)))
