...
from PySide6.QtWidgets import QCheckBox, QComboBox, QFormLayout, QGroupBox, QLabel, QLineEdit, QMenu, QPlainTextEdit, \
...
def _compatible_shape(self, draw, svg, cfg, rng, shape, cx, cy, size, rotation, color, alpha):
    ...
    if shape in {"block", "tri"}:
        ...
        for i in range(len(front)):
            ...
            svg.append(
                f'<polygon points="{a[0]:.1f},{a[1]:.1f} {b[0]:.1f},{b[1]:.1f} {bj[0]:.1f},{bj[1]:.1f} {bi[0]:.1f},{bi[1]:.1f}" fill="rgb({shadow[0]},{shadow[1]},{shadow[2]})" fill-opacity="{face_alpha / 255:.3f}"/>')
        draw.polygon(back, fill=(*shadow, shadow_alpha));
        svg.append(
            f'<polygon points="{" ".join(f"{x:.1f},{y:.1f}" for x, y in back)}" fill="rgb({shadow[0]},{shadow[1]},{shadow[2]})" fill-opacity="{shadow_alpha / 255:.3f}"/>')
    elif shape == "circle":
        r = size * .5;
        draw.ellipse((cx - r + ox, cy - r + oy, cx + r + ox, cy + r + oy), fill=(*shadow, shadow_alpha));
        svg.append(
            f'<ellipse cx="{cx + ox:.1f}" cy="{cy + oy:.1f}" rx="{r:.1f}" ry="{r:.1f}" fill="rgb({shadow[0]},{shadow[1]},{shadow[2]})" fill-opacity="{shadow_alpha / 255:.3f}"/>')
    elif shape == "line":
        half = size * .5;
        ...
        for i in range(count):
            t = i / max(1, count - 1);
            front.append((cx - half + t * size, cy + math.sin(t * math.pi * 2 + rotation) * size * .25))
        ...
        svg.append(
            f'<polyline points="{" ".join(f"{x:.1f},{y:.1f}" for x, y in back)}" fill="none" stroke="rgb({shadow[0]},{shadow[1]},{shadow[2]})" stroke-opacity="{shadow_alpha / 255:.3f}" stroke-width="{width}" stroke-linecap="round"/>')
    ...
    if edge_light > 0 and shape in {"block", "tri", "circle"}:
        ...
        ha = max(12, int(alpha * .20));
        r = size * .5;
        draw.ellipse((cx - r * .48, cy - r * .76, cx - r * .20, cy - r * .48), fill=(*hi, ha));
        svg.append(
            f'<ellipse cx="{cx - r * .34:.1f}" cy="{cy - r * .62:.1f}" rx="{r * .14:.1f}" ry="{r * .14:.1f}" fill="rgb({hi[0]},{hi[1]},{hi[2]})" fill-opacity="{ha / 255:.3f}"/>')
...
def _font_families() -> list[str]:
    try:
        ...
        all_families = set(); japanese = set()
    preferred = ["Yu Mincho", "YuMincho", "MS Mincho", "MS PMincho", "BIZ UDPMincho", "BIZ UDMincho",
                 "Noto Serif CJK JP", "Noto Serif JP", "Source Han Serif", "Source Han Serif JP",
                 "Hiragino Mincho ProN", "Hiragino Mincho Pro", "IPAexMincho", "IPAMincho", "Yu Gothic", "YuGothic",
                 "Meiryo", "Meiryo UI", "MS Gothic", "MS PGothic", "BIZ UDPGothic", "BIZ UD Gothic", "Noto Sans CJK JP",
                 "Noto Sans JP", "Source Han Sans", "Source Han Sans JP", "Hiragino Kaku Gothic ProN", "IPAexGothic",
                 "IPAGothic", "Bahnschrift", "Bahnschrift SemiBold", "Segoe UI", "Segoe UI Variable", "Segoe UI Light",
                 "Aptos", "Aptos Display", "Arial", "Arial Narrow", "Helvetica Neue", "Futura", "Gill Sans", "Garamond",
                 "Book Antiqua", "Century Gothic", "Impact", "Trebuchet MS", "Consolas", "Cascadia Code",
                 "Cascadia Mono", "JetBrains Mono", "IBM Plex Sans", "IBM Plex Serif", "Inter", "Montserrat", "Roboto",
                 "Roboto Condensed"]
    ordered = [];
    seen = set()
    ...
    keywords = (
    "display", "condensed", "mono", "serif", "gothic", "mincho", "script", "slab", "hand", "sans", "headline", "black",
    "light", "variable", "retro", "pixel")
...
def _vertical_grid_text_extension(self, result, config):
    ...
    char_h = metrics.height() * float(settings["line_spacing"]) + float(settings.get("tracking", 0))
    chars = [c for c in text if c != "\n"];
    rng = random.Random(f"{result.seed}:text");
    palette = self._palette(rng, config);
    alpha = max(1, min(255, int(255 * settings["opacity"])));
    svg = []
    cols = max(1, int(config.width / max(24, settings["font_size"] * 1.35)));
    rows = max(1, int(config.height / max(24, char_h)));
    positions = [];
    used = 0
    for col in range(cols):
        ...
        for row in range(rows):
            ...
            char = chars[used];
            used += 1;
            y = settings["font_size"] * 0.8 + row * char_h;
            positions.append((char, x, y))
        if used >= len(chars): break
    margin = max(12, float(settings["font_size"]) * 0.45)
    if positions:
        bw = max(x for _, x, _ in positions) - min(x for _, x, _ in positions) + settings["font_size"];
        bh = max(y for _, _, y in positions) - min(y for _, _, y in positions) + char_h
        target = _anchor_rect(settings.get("anchor", "free"), config.width, config.height, bw, bh, margin)
        if target:
            dx = target[0] - (min(x for _, x, _ in positions) - settings["font_size"] / 2);
            dy = target[1] - (min(y for _, _, y in positions) - char_h / 2);
            positions = [(c, x + dx, y + dy) for c, x, y in positions]

    def paint(char, x, y):
        color = self._choose_color(rng, palette, config, x, y);
        painter.save();
        painter.translate(x, y);
        painter.setPen(QPen(QColor(*color, alpha)));
        painter.drawText(QPointF(0, (metrics.ascent() - metrics.descent()) / 2), char);
        painter.restore();
        svg.append(
            f'<text x="{x:.1f}" y="{y:.1f}" font-family="{html.escape(settings["font_family"])}" font-size="{int(settings["font_size"])}" fill="rgb({color[0]},{color[1]},{color[2]})" fill-opacity="{alpha / 255:.3f}" text-anchor="middle" dominant-baseline="middle" xml:space="preserve">{html.escape(char)}</text>')
...
def _shift_svg_text(svg, dx, dy):
    def repl(match):
        ...
        attrs = match.group(3)
        attrs = re.sub(r'rotate\((-?[\d.]+)\s+(-?[\d.]+)\s+(-?[\d.]+)\)',
                       lambda m: f'rotate({m.group(1)} {float(m.group(2)) + dx:.2f} {float(m.group(3)) + dy:.2f})',
                       attrs)
...
def _draw_text_extension(self, result, config):
    ...
    base_image = result.image.convert("RGBA");
    base_svg = result.svg
    result.image = Image.new("RGBA", (config.width, config.height), (0, 0, 0, 0));
    result.svg = base_svg
    _original_text_extension(self, result, config)
    text_layer = result.image;
    text_svg = result.svg[len(base_svg):]
    bbox = text_layer.getbbox()
    if not bbox:
        result.image = base_image;
        result.svg = base_svg;
        return
    l, t, r, b = bbox;
    bw = r - l;
    bh = b - t;
    margin = max(12, int(self.text_settings.get("font_size", 72) * 0.35));
    target = _anchor_rect(anchor, config.width, config.height, bw, bh, margin)
    if target:
        dx = int(round(target[0] - l));
        dy = int(round(target[1] - t))
        shifted = Image.new("RGBA", text_layer.size, (0, 0, 0, 0));
        shifted.alpha_composite(text_layer, (dx, dy));
        text_layer = shifted;
        text_svg = _shift_svg_text(text_svg, dx, dy)
    result.image = Image.alpha_composite(base_image, text_layer);
    result.svg = base_svg + text_svg


extensions.EnhancedPatternRenderer._draw_text_extension = _draw_text_extension

EN_TRANSLATIONS = {
    "Settings": "設定", "File": "ファイル", "Generate": "生成", "Save PNG": "PNGを保存", "Save SVG": "SVGを保存",
    "Save preset": "プリセットを保存", "Load preset": "プリセットを読み込む",
    "Composition": "構成", "Field": "フィールド", "Geometry": "ジオメトリ", "Color": "カラー", "Layers": "レイヤー",
    "Behavior": "動作", "Organic": "有機", "Text": "テキスト", "Export": "書き出し",
    "Canvas": "キャンバス", "Width": "幅", "Height": "高さ", "Aspect": "比率", "Apply aspect": "比率を適用",
    "Seed": "シード", "Background": "背景", "Palette": "パレット", "Symmetry": "対称",
    "Spatial composition": "空間構成", "Mode": "モード", "Focal X": "焦点X", "Focal Y": "焦点Y",
    "Focal strength": "焦点強度", "Edge bias": "エッジ偏り", "Cluster count": "クラスタ数",
    "Cluster strength": "クラスタ強度", "Spacing": "間隔", "Position jitter": "位置ジッター",
    "Vector field": "ベクトルフィールド", "Strength": "強度", "Scale": "スケール", "Curvature": "曲率",
    "Steps": "ステップ数", "Step size": "ステップ幅", "Octaves": "オクターブ",
    "Primitive probability": "プリミティブ確率", "Blocks": "ブロック", "Circles": "円", "Lines": "線",
    "Triangles": "三角形", "weight": "重み",
    "Color behavior": "カラー設定", "Palette size": "パレット数", "Saturation": "彩度", "Contrast": "コントラスト",
    "Hue jitter": "色相ジッター", "Opacity min": "最小不透明度", "Opacity max": "最大不透明度",
    "Color coherence": "色の一貫性",
    "Depth & surface": "奥行き・表面", "Layer count": "レイヤー数", "Depth": "奥行き",
    "Accent density": "アクセント密度", "Gradient background": "グラデーション背景", "Raster blur": "ラスターブラー",
    "Controlled behavior": "動作制御", "Behavior preset": "動作プリセット", "Mutation": "変異", "Asymmetry": "非対称",
    "Randomize system": "システムをランダム化",
    "Export current result": "現在の結果を書き出す", "Generate / Refresh": "生成 / 更新",
    "Save preset JSON": "プリセットJSONを保存", "Load preset JSON": "プリセットJSONを読み込む",
    "Organic forms": "有機形状", "Enable organic forms": "有機形状を有効化", "Style": "スタイル", "Weight": "重み",
    "Lobes": "ローブ数", "Wobble": "揺らぎ", "Taper": "テーパー", "Linework": "線描", "Vein count": "葉脈数",
    "Vein wobble": "葉脈の揺らぎ", "Long strands": "長いストランド数", "Strand length": "ストランド長",
    "Strand wander": "ストランドの蛇行", "Strand width": "ストランド幅",
    "Perspective & depth": "遠近感・奥行き", "Enable dimensional depth": "立体的奥行きを有効化",
    "Vanishing X": "消失点X", "Vanishing Y": "消失点Y", "Edge light": "エッジライト",
    "Text / Unicode overlay": "テキスト / Unicode オーバーレイ", "Enable text": "テキストを有効化",
    "Arrangement": "配置", "Font": "フォント", "Size": "サイズ", "Tracking": "トラッキング", "Line spacing": "行間",
    "Radius": "半径", "Wave amount": "波の量", "Rotation": "回転",
    "System settings": "システム設定", "Language": "言語", "English": "English", "Japanese": "日本語",
    "Reset window size": "ウィンドウサイズを初期化",
    "Snap position": "スナップ位置", "Free / current": "自由 / 現在位置", "Top left": "左上", "Top center": "上中央",
    "Top right": "右上", "Center left": "中央左", "Center": "中央", "Center right": "中央右", "Bottom left": "左下",
    "Bottom center": "下中央", "Bottom right": "右下",
    "Procedural composition laboratory": "プロシージャル構成ラボ", "Generate a pattern": "パターンを生成"
}
JA_TO_EN = {v: k for k, v in EN_TRANSLATIONS.items()}
ANCHORS = [("free / current", "自由 / 現在位置"), ("top-left", "左上"), ("top-center", "上中央"), ("top-right", "右上"),
           ("center-left", "中央左"), ("center", "中央"), ("center-right", "中央右"), ("bottom-left", "左下"),
           ("bottom-center", "下中央"), ("bottom-right", "右下")]
...
class MainWindow(extensions.MainWindow):
    ...
    def __init__(self):
        ...
        self._add_vertical_text_mode();
        self._add_text_anchor_control();
        self._install_settings_menu();
        self._apply_language(self.settings.value("language", "en"))
    ...
    def _populate_text_fonts(self):
        ...
        combo.setCurrentText(
            current if current in families else ("Bahnschrift" if "Bahnschrift" in families else families[0]));
    ...
    def _add_text_anchor_control(self):
        ...
        self.text_anchor.setCurrentIndex(0);
        self.text_anchor.currentIndexChanged.connect(self._request)
    ...
    def _config(self):
        ...
        cfg = super()._config()
        if hasattr(self, "text_anchor"): self.renderer.text_settings[
            "anchor"] = self.text_anchor.currentData() or "free / current"
        return cfg
    ...
    def _install_settings_menu(self):
        ...
        self._lang_en = lang_menu.addAction("English");
        self._lang_ja = lang_menu.addAction("日本語")
        ...
        self._lang_ja.triggered.connect(lambda: self._set_language("ja"))
        settings_menu.addSeparator();
        reset = settings_menu.addAction("Reset window size");
        reset.triggered.connect(self._reset_window_size)
    ...
    def _apply_language(self, language):
        ...
        for widget in self.findChildren((QLabel, QPushButton, QCheckBox, QLineEdit, QPlainTextEdit)):
            ...
            try:
                widget.setText(self._translate_text(widget.text(), language))
            except Exception:
                pass
            ...
            for layout in self.findChildren(QFormLayout):
                for row in range(layout.rowCount()):
                    ...
                    if label is not None and label.widget() is not None:
                        w = label.widget()
                        if w.text() == "Snap position" or w.text() == "スナップ位置": w.setText(
                            self._translate_text("Snap position", language))
        for action in self.menuBar().actions(): action.setText(self._translate_text(action.text(), language))
        self._settings_menu.setTitle(self._translate_text("Settings", language)) if hasattr(self,
                                                                                            "_settings_menu") else None
