...
def _p(key,name,hexes):
    return Palette(key,name,tuple(_rgb(x) for x in hexes.split()))

# 43-flag curated baseline; values transcribed from TRezendes/pride_palette.
DATA = [
...
PRIDE_PALETTES={k:_p(k,n,v) for k,n,v in DATA}
PRIDE_PALETTES.update({'ace':PRIDE_PALETTES['asexual'],'aro':PRIDE_PALETTES['aromantic'],'bi':PRIDE_PALETTES['bisexual'],'pan':PRIDE_PALETTES['pansexual'],'trans':PRIDE_PALETTES['transgender'],'gwen':PRIDE_PALETTES['lesbian'],'howell':PRIDE_PALETTES['polyamory-heart'],'evans':PRIDE_PALETTES['polyamory']})
BUILTIN_PALETTES={
'neon':_p('neon','Neon','ff3278 3cffb4 5a78ff ffdb3c e150ff'),
'earth':_p('earth','Earth','523828 9b6e46 d6bb8c 485d3e 1e1e1e'),
'mono':_p('mono','Monochrome','1e1e1e 5a5a5a 969696 dcdcdc'),
'ice':_p('ice','Ice','d2f0ff 82cdf0 4696c8 ebfaff 2d5578'),
'ritual':_p('ritual','Ritual','181212 641e1e a0372d d78c46 ebd296')}
ALL_PALETTES={**BUILTIN_PALETTES,**PRIDE_PALETTES}
ALL_LGBTQ_COLORS=tuple(dict.fromkeys(c for k,n,v in DATA for c in PRIDE_PALETTES[k].colors))

def canonical_keys():
    return list(BUILTIN_PALETTES)+[x[0] for x in DATA]
